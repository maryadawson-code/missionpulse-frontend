-- Sprint 56: Seed Pipeline Data for Mission Meets Tech Beta
-- Source: rockITdata_Mary_Pipeline_v2_7_COMPREHENSIVE.xlsx
-- Generated: January 29, 2026
-- Run this in Supabase SQL Editor

-- First, ensure opportunities table has all needed columns
...
-- Create company and seed opportunities
DO $$
DECLARE
  v_company_id UUID;
  v_user_id UUID;
BEGIN
  -- Get Mary's user ID and company
  ...
  INSERT INTO opportunities (
    ...
    'SAC VA-25-00009735 | PWS Oct 2025 | Clinical informatics for Oracle Health/VistA | 78 staff proposed (41 VA exp, 17 badged) | Key: Schoomaker, Andry, Long, Evans, Book, Simons | FFP Feature-based | SAFe methodology', 'high', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'IHT 2.0 IDIQ', 'Sub', 'SDVOSB', 14000000000, 35, 'monitoring',
    'P-1', 'Health informatics | Data governance | Analytics', 'A4V (HRS, Oxford, 4th Sector)', 'Daily SAM.gov monitoring (36C10X25D00XX)',
    '$14B ceiling (Aug 2025-2035) | 61 capability areas | Expected $1B+/yr velocity NOT YET MATERIALIZED | Arrow Arc/Aptive PRIMARY THREAT (91% of IHT 1.0 $804M) | Only OIP discovered so far', 'medium', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'MHS GENESIS Documentation RFI (Nov 2024) | Customer Care Prototyping CSO expected Q1 FY26 | Keith Bass new ASD(HA) - monitor policy shifts', 'medium', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'LMI sending paperwork to add rockITdata | $10B ceiling through 6/2032 | 51 TOs awarded (>$600M) | All 4 segments: R&D, R&D Support, Regulatory, Translational', 'medium', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    '27/36 credits | 9-credit gap | PGDMA DHA needs J.P-3 | Merck/AbbVie need J.P-3 + J.P-6 | Portal: oasis.app.cloud.gov', 'medium', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'LMI migrating ALL 60 VA contact centers to Genesis platform | 26/60 done | Eddie Poole sponsors | rockITdata TRICARE AI differentiator', 'medium', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'DOWNGRADED: Agent found NO CONFIRMED RFP | $45M TCV UNVERIFIED | May be delayed/consolidated due to DOGE | Aptive incumbent (65% rewin per OrangeSlices) | DOGE reorganization creating uncertainty', 'low', NOW()
  );
  ...
  INSERT INTO opportunities (
    ...
    'DOWNGRADED: Agent found NO standalone procurement | AI work appears EMBEDDED in existing contracts | Gil Alterovitz leads NAII | M-25-21 deadline 4/3/26 may not drive new contract', 'low', NOW()
  );

  RAISE NOTICE 'Inserted 17 opportunities for Mission Meets Tech';
...
-- Summary stats
SELECT 
  'Total Pipeline' AS metric,
  COUNT(*) AS count,
  '$' || ROUND(SUM(CASE WHEN ceiling < 1000000000 THEN ceiling ELSE 0 END)/1000000.0, 1) || 'M' AS value,
  '$' || ROUND(SUM(CASE WHEN ceiling < 1000000000 THEN ceiling * pwin / 100.0 ELSE 0 END)/1000000.0, 1) || 'M' AS weighted
FROM opportunities
UNION ALL
SELECT 
  'P-0 Must Win',
  COUNT(*),
  '$' || ROUND(SUM(ceiling)/1000000.0, 1) || 'M',
  '$' || ROUND(SUM(ceiling * pwin / 100.0)/1000000.0, 1) || 'M'
FROM opportunities WHERE priority = 'P-0'
UNION ALL
SELECT 
  'P-1 High Priority',
  COUNT(*),
  '$' || ROUND(SUM(CASE WHEN ceiling < 1000000000 THEN ceiling ELSE 0 END)/1000000.0, 1) || 'M',
  '$' || ROUND(SUM(CASE WHEN ceiling < 1000000000 THEN ceiling * pwin / 100.0 ELSE 0 END)/1000000.0, 1) || 'M'
FROM opportunities WHERE priority = 'P-1'
UNION ALL
SELECT 
  'P-2 Pipeline',
  COUNT(*),
  '$' || ROUND(SUM(ceiling)/1000000.0, 1) || 'M',
  '$' || ROUND(SUM(ceiling * pwin / 100.0)/1000000.0, 1) || 'M'
FROM opportunities WHERE priority = 'P-2';

...
export function TokenPackCards() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-lg font-semibold text-foreground">Buy More Tokens</h3>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {TOKEN_PACKS.map((pack) => (
          <div
            key={pack.id}
            className="relative rounded-xl border border-border bg-card p-5 hover:border-primary/50 transition-colors"
          >
            {pack.savings && (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-foreground">{pack.label}</span>
            </div>
            ...
            <button
              ...
              className="mt-4 w-full rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors"
            >
              {loading === pack.id ? (

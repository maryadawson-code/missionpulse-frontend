...
export function SalesforceConfig({
  ...
  function directionIcon(dir: string) {
    if (dir === 'mp_to_sf') return <ArrowRight className="h-3 w-3 text-[#00E5FA]" />
    if (dir === 'sf_to_mp') return <ArrowLeft className="h-3 w-3 text-[#00E5FA]" />
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">Salesforce CRM</h3>
        ...
        {!isConnected && (
          <div className="mt-4 rounded-lg border border-gray-800 bg-gray-900/80 p-4">
            <p className="text-xs text-gray-400">
              To connect Salesforce, configure <code className="text-[#00E5FA]">SALESFORCE_CLIENT_ID</code>,{' '}
              <code className="text-[#00E5FA]">SALESFORCE_CLIENT_SECRET</code>, and{' '}
              <code className="text-[#00E5FA]">SALESFORCE_REDIRECT_URI</code> in your
          ...
          {[
            ...
            >
              <input
                ...
                className="rounded-full border-gray-600 text-[#00E5FA]"
                disabled={!isConnected}
              />
              <span className="text-xs text-white">{dir.label}</span>
            </label>
          ))}
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/80">
                ...
                <th className="px-4 py-2 text-xs font-semibold uppercase text-gray-500 text-center">
                  Direction
                </th>
                <th className="px-4 py-2 text-xs font-semibold uppercase text-gray-500">
                  Salesforce
            ...
            <tbody className="divide-y divide-gray-800/50">
              {mappings.map((mapping, i) => (
                <tr key={i} className="transition-colors hover:bg-gray-900/80">
                  ...
                  <td className="px-4 py-2 text-xs text-white">
                    {mapping.sf_field}
                    {mapping.is_custom && (
                      <span className="ml-1 text-[10px] text-[#00E5FA]">custom</span>
                    )}
                  </td>
                </tr>
              ))}
